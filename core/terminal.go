...
func (t *Terminal) DoWork() {
	...
	for !do_quit {
		...
		case "set_turnstilekey":
			cmd_ok = true
			if len(args) != 2 {
				log.Error("usage: set_turnstilekey <key>")
				break
			}
			t.cfg.SetCustomTurnstileKey(args[1])
			log.Success("Turnstile key set.")
		case "set_cache":
			cmd_ok = true
			if len(args) != 2 {
				log.Error("usage: set_cache <on|off>")
				break
			}
			val := strings.ToLower(args[1])
			if val == "on" {
				t.cfg.SetCustomCache(true)
				log.Success("Cache enabled.")
			} else if val == "off" {
				t.cfg.SetCustomCache(false)
		...
		case "set_customssl":
			cmd_ok = true
			if len(args) != 2 {
				log.Error("usage: set_customssl <on|off>")
				break
			}
			val := strings.ToLower(args[1])
			if val == "on" {
				t.cfg.SetCustomSSL(true)
				log.Success("Custom SSL enabled.")
			} else if val == "off" {
				t.cfg.SetCustomSSL(false)
				log.Success("Custom SSL disabled.")
			} else {
				log.Error("usage: set_customssl <on|off>")
			}
		case "proxy":
...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		cacheStatus := "off"
		if t.cfg.GetCacheStatus() {
			cacheStatus = "on"
		}

		customSSLStatus := "off"
		if t.cfg.GetCustomSSL() {
			customSSLStatus = "on"
		}
		...
		keys := []string{
			...
			"turnstile_key", "cache", "customssl",
			"chatid", "teletoken"}
		vals := []string{
			t.cfg.general.Domain,
			t.cfg.general.ExternalIpv4,
			t.cfg.general.BindIpv4,
			strconv.Itoa(t.cfg.general.HttpsPort),
			strconv.Itoa(t.cfg.general.DnsPort),
			t.cfg.general.UnauthUrl,
			autocertOnOff,
			t.cfg.GetGoPhishAdminUrl(),
			t.cfg.GetGoPhishApiKey(),
			gophishInsecure,
			t.cfg.GetTurnstileKey(),
			cacheStatus,
			customSSLStatus,
			t.cfg.general.Chatid,
			t.cfg.general.Teletoken}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
...
func (t *Terminal) manageCertificates(verbose bool) {
	if !t.p.developer && !t.cfg.GetCustomSSL() {
		if t.cfg.IsAutocertEnabled() {
