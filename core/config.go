...
type CustomConfig struct {
	...
	Customssl    bool   `mapstructure:"customssl" json:"customssl" yaml:"customssl"`
...
func NewConfig(cfg_dir string, path string) (*Config, error) {
	...
	if _, err := os.Stat(path); os.IsNotExist(err) {
		created_cfg = true
		// Set initial values for custom config
		c.custom.TurnstileKey = ""
		c.custom.Cache = false
		c.custom.Customssl = false // Set initial value of customssl to off (false)
		c.cfg.Set(CFG_CUSTOM, c.custom)
	...
	c.cfg.UnmarshalKey(CFG_CUSTOM, &c.custom)
...
func (c *Config) SetCustomTurnstileKey(key string) {
	c.custom.TurnstileKey = key
	c.cfg.Set(CFG_CUSTOM, c.custom)
	log.Info("custom turnstile key set to: %s", key)
	c.cfg.WriteConfig()
}

func (c *Config) GetCustomTurnstileKey() string {
	return c.custom.TurnstileKey
}

func (c *Config) SetCustomCache(enabled bool) {
	c.custom.Cache = enabled
	c.cfg.Set(CFG_CUSTOM, c.custom)
	log.Info("custom cache set to: %v", enabled)
	c.cfg.WriteConfig()
}

func (c *Config) GetCustomCache() bool {
	return c.custom.Cache
}

func (c *Config) GetTurnstileKey() string {
	return c.custom.TurnstileKey
}

func (c *Config) GetCacheStatus() bool {
	return c.custom.Cache
}

// Add getter and setter for CustomSSL in Config
func (c *Config) SetCustomSSL(enabled bool) {
	c.custom.Customssl = enabled
	c.cfg.Set(CFG_CUSTOM, c.custom)
	log.Info("custom SSL set to: %v", enabled)
	c.cfg.WriteConfig()
}

func (c *Config) GetCustomSSL() bool {
	return c.custom.Customssl
}
