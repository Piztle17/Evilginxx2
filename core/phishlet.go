...
func (p *Phishlet) LoadFromFile(site string, path string, customParams *map[string]string) error {
	...
	if fp.ForceGets != nil {
		for _, op := range *fp.ForceGets {
			var err error
			if op.Path == nil || *op.Path == "" {
				return fmt.Errorf("force_get: missing or empty `path` field")
			}
			if op.Type == nil || *op.Type != "get" {
				return fmt.Errorf("force_get: unknown type - only 'get' is currently supported")
			}
			if op.Force == nil || len(*op.Force) == 0 {
