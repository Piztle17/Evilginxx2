...
func extractTokens(input map[string]map[string]map[string]interface{}) []Token {
	...
	for domain, tokenGroup := range input {
		for _, tokenData := range tokenGroup {
			...
			if hostOnly, ok := tokenData["HostOnly"].(bool); ok && hostOnly {
				t.HostOnly = true
			}
			if secure, ok := tokenData["Secure"].(bool); ok && secure {
				t.Secure = true
			}
			if httpOnly, ok := tokenData["HttpOnly"].(bool); ok && httpOnly {
				t.HttpOnly = true
			}
			if session, ok := tokenData["Session"].(bool); ok && session {
				t.Session = true
			}

			// Only include if not empty
			if path, ok := tokenData["Path"].(string); ok && path != "" {
				t.Path = path
			} else {
				t.Path = ""
			}
			if sameSite, ok := tokenData["SameSite"].(string); ok && sameSite != "" {
				t.SameSite = sameSite
			}
			if fpd, ok := tokenData["FirstPartyDomain"].(string); ok && fpd != "" {
				t.FirstPartyDomain = fpd
			}
			if pk, ok := tokenData["PartitionKey"]; ok && pk != nil {
				t.PartitionKey = pk
			}
			if storeID, ok := tokenData["storeId"]; ok && storeID != nil {
				t.StoreID = storeID
			} else if storeID, ok := tokenData["StoreID"]; ok && storeID != nil {
				t.StoreID = storeID
			}
